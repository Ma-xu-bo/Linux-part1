...
extern void Error_405(struct worker_ctl* wctl);
...
int Request_Parse(struct worker_ctl *wctl)
{
  ...
  struct conn_response *res = &c->conn_res;
  int retval = 200;
  ...
  *pos = '\0';
  ...
  for(m = &_shttpd_methods[0];m->ptr!=NUL;m++)
    {
 ...
 EXITRequest_Parse:
  DBGPRINT("<==Reauesst_Parse\n");
  return retval;
}

int Request_Headle(struct worker_ctl* wctl)
{
  ...
  switch(err)
    ...
    case 400:
      Error_400(wctl);
      cl = wctl->conn.con_res.cl;
      len = strlen(wctl->conn.con_res.ptr);
